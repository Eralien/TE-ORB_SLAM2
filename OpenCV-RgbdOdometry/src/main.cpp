...
vector<double> toQuaternion(const Mat &M)
{
...
void writeToFile(ostream & outFile, Mat &rotation, Mat &trans, const string &timestamp)
{
    auto q = toQuaternion(rotation);

    outFile << setprecision(6) << timestamp << " "
            << setprecision(9) << trans.at<double>(0) << " "
            << trans.at<double>(1) << " "
            << trans.at<double>(2) << " "
...
bool testRGBD(const std::vector<std::string>& inputRGBPaths,
  ...
  for (int i = 0; i != inputRGBPaths.size() - 1; i++) {
    ...
    depth0.convertTo(depthFlt0, CV_32FC1, CameraParams::PIXEL_TO_METER_SCALEFACTOR);
    depth1.convertTo(depthFlt1, CV_32FC1, CameraParams::PIXEL_TO_METER_SCALEFACTOR);
    // Read images end
    ...
    if (!odom) {
      ...
      odom = std::make_shared<rgbd::RgbdOdometry>(
          rgbodo.camera_matrix, CameraParams::MIN_DEPTH, CameraParams::MAX_DEPTH, CameraParams::MAX_DEPTH_DIFF, iterCounts,
    ...
    */
    ...
    bool isSuccess = rgbodo.odom->compute(grayImage0, depthFlt0, Mat(), grayImage1,
    ...
    writeToFile(outFile, rotationMatrix, tranlslationMatrix, timestamp[i+1]);
    ...
    depthFlt1.convertTo(normalizeDepth, CV_8UC1, 255.0 / CameraParams::MAX_DEPTH);
...
int main(int argc, char** argv) {
  ...
  std::vector<std::string> inputRGBPaths, inputDepthPaths, timestamp;
  // readAssocTextfile("../data/fr1_room.txt", inputRGBPaths, inputDepthPaths, timestamp);
  readAssocTextfile("fr2_pioneer_360.txt", inputRGBPaths, inputDepthPaths, timestamp);
  std::cerr << "test opencv rgbd, end loading assoc.txt" << std::endl;
  if (testRGBD(inputRGBPaths, inputDepthPaths, timestamp)) {
    return 0;
  }
  return -1;
}
